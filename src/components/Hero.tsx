...
export default function Hero() {
  return (
    <div id="hero" className="flex flex-col lg:flex-row h-auto lg:h-[100dvh] w-full bg-bg-lightgreen font-serif text-text-dark lg:overflow-hidden relative pb-12 lg:pb-0">
      <img src={heroBg} alt="Background" className="absolute inset-0 w-full h-full object-cover z-0 opacity-40 pointer-events-none" />
        ...
        <div className="w-full max-w-[1200px] mx-auto pt-24 lg:pt-0 flex-1 flex flex-col lg:flex-row gap-10 xl:gap-20 items-center justify-center">
          ...
          <div className="w-full lg:flex-1 flex flex-col justify-center text-center lg:text-left">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4 justify-center lg:justify-start">
              <button className="w-full sm:w-auto font-sans text-xs lg:text-sm font-bold px-8 py-4 bg-brand-dark text-white rounded-full hover:bg-brand-teal transition-all shadow-lg active:scale-95 tracking-widest uppercase">
                Book Assessment
              </button>
              <button className="w-full sm:w-auto font-sans text-xs lg:text-sm font-bold px-8 py-4 bg-white/50 backdrop-blur-md text-brand-dark rounded-full hover:bg-white transition-all active:scale-95 border border-white/40 tracking-widest uppercase">
                Learn More
              </button>
            </div>
          </div>
            ...
            <div className="w-full bg-white/30 backdrop-blur-xl rounded-[40px] p-8 lg:p-10 flex flex-col justify-center relative border border-white/40 shadow-[0_30px_60px_rgba(47,93,80,0.08)] overflow-hidden group/box">
              ...
              <div className="relative z-10">
                ...
                <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-4 w-full">
                  {[
                    { label: "Botox & Fillers", sub: "Natural enhancement" },
                    { label: "Medical Weight Loss", sub: "GLP-1 Protocols" },
                    { label: "Pico & Laser", sub: "Skin rejuvenation" },
                    { label: "Hair Restoration", sub: "PRP & Cell therapy" },
                  ].map((item, i) => (
      ...
      <div className="w-[110px] xl:w-[130px] h-full bg-bg-lightgreen/80 backdrop-blur-md border-l border-white/20 hidden lg:flex flex-col justify-evenly pt-24 pb-16 px-2 items-center text-center shrink-0 z-30 relative shadow-[-20px_0_40px_rgba(0,0,0,0.02)]">
        {[
          ...
          <div key={i} className="flex flex-col items-center gap-1 group">
            ...
            {i < 3 && <div className="w-8 h-[1px] bg-brand-teal/20 mt-4"></div>}
          </div>
        ))}
      </div>
    </div>
